...
import '../lib/bloc/goal/goal_bloc.dart';
import '../lib/bloc/goal/goal_event.dart';
import '../lib/bloc/goal/goal_state.dart';
...
class DrawerSyncValidator {
  ...
  static Future<bool> validateDeleteGoalSync({
    ...
    // 2. 模拟删除目标
    final updatedGoals = initialGoals.where((g) => g.id != goalToDelete.id).toList();
    ...
    for (final goal in allGoals) {
      ...
      for (final subGoal in goal.subGoals) {
        ...
        if (expectedSubLevel != actualSubLevel) {
          print('❌ 子目标层级错误: ${subGoal.title} 期望层级:$expectedSubLevel 实际层级:$actualSubLevel');
          allCorrect = false;
        }
      }
    }
  ...
  static Future<bool> validateBlocStateSync({
    ...
    if (blocState.currentGoal != null) {
      final currentGoalInDrawer = drawerGoals.any((g) => g.id == blocState.currentGoal!.id);
